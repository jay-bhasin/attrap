...
def __main__():
    ...
    stdp_kernel = 1e-2*np.exp(-(t_states-0.12)**2/(2*std_states**2))

    if not os.path.isdir(os.getcwd()+'/results'):
        os.makedirs(os.getcwd()+'/results')
    if not os.path.isdir(os.getcwd()+'/results/tuned'):
        os.makedirs(os.getcwd()+'/results/tuned')

    for n in tqdm.trange(N_sims):
        w, pc_averaged, errors, sses = behaviorLearning(N_iterations, stdp_kernel, baseline_gain=0.35)

        np.save(os.getcwd() + '/results/tuned/%d_pc_tuned.npy'%n, pc_averaged)
        np.save(os.getcwd() +'/results/tuned/%d_sse_tuned.npy'%n, sses)
        np.save(os.getcwd() +'/results/tuned/%d_w_tuned.npy'%n, w)
